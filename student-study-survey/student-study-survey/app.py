...
def init_storage():
    """Initialize SQLite database, JSON, and CSV storage."""
...
def read_responses():
    """Read all responses reliably from SQLite (fallback to JSON)."""
    try:
...
def save_response(data):
    """Permanently store in SQLite, JSON, and CSV atomically."""
    # 1. Save to SQLite
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
...
def create_styled_portrait_excel(responses):
    ...
    for idx, r in enumerate(responses):
        ws.row_dimensions[current_row].height = 65 # Generous height for readable portrait cards

        # Format Class
        c_disp = r.get("q1_class", "")
        if c_disp.lower() == "above" and r.get("q1_class_other"):
            c_disp = f"Above 12th\n({r.get('q1_class_other')})"

        # Format Difficult Subjects & Why
    ...
    # Auto-adjust column widths for Portrait Layout
    col_widths = {
        1: 18, # ID & Time
        2: 20, # Student Name
        3: 24, # Email
        4: 16, # Class
        5: 32, # Difficult Subjects
        6: 30, # Biggest Problem
        7: 28, # When Stuck
        8: 28, # Weak Topics
        9: 30, # Feedback & HW
        10: 30 # App Interest
